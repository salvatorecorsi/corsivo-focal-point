...
function corsivo_focal_point_migrate_post( $post_id ) {
	...
	if ( ! metadata_exists( 'post', $post_id, CORSIVO_FOCAL_POINT_META_X ) ) {
		$success = corsivo_focal_point_write_single_meta( $post_id, CORSIVO_FOCAL_POINT_META_X, 50 ) && $success;
	}

	if ( ! metadata_exists( 'post', $post_id, CORSIVO_FOCAL_POINT_META_Y ) ) {
		$success = corsivo_focal_point_write_single_meta( $post_id, CORSIVO_FOCAL_POINT_META_Y, 50 ) && $success;
	}
	...
	return corsivo_focal_point_write_single_meta( $post_id, CORSIVO_FOCAL_POINT_META_ATTACHMENT, get_post_thumbnail_id( $post_id ) ) && $success;
}
...
function corsivo_focal_point_update_position( $post_id, $x, $y, $attachment_id ) {
	...
	foreach ( array_keys( $values ) as $meta_key ) {
		$previous_values[ $meta_key ] = corsivo_focal_point_get_raw_meta_values( $post_id, $meta_key );
	}
	...
	foreach ( $previous_values as $meta_key => $meta_values ) {
		if ( $meta_values ) {
			corsivo_focal_point_write_single_meta( $post_id, $meta_key, reset( $meta_values ) );
		} elseif ( 1 === count( corsivo_focal_point_get_raw_meta_values( $post_id, $meta_key ) )
			&& (string) get_post_meta( $post_id, $meta_key, true ) === (string) $values[ $meta_key ]
		) {
			delete_metadata( 'post', $post_id, $meta_key, $values[ $meta_key ] );
		}
	}
...
function corsivo_focal_point_mark_revision( $revision_id, $post_id ) {
	...
	if ( ! wp_is_post_autosave( $revision_id )
		...
		update_metadata( 'post', $revision_id, CORSIVO_FOCAL_POINT_REVISION_MARKER, corsivo_focal_point_revision_marker_value( $revision_id ) );
	}
}
...
function corsivo_focal_point_mark_rest_autosave( $response, $autosave, $request ) {
	...
	update_metadata( 'post', $autosave->ID, CORSIVO_FOCAL_POINT_REVISION_MARKER, corsivo_focal_point_revision_marker_value( $autosave->ID ) );

	if ( ! corsivo_focal_point_revision_is_compatible( $autosave->ID ) ) {
		return new WP_Error(
			'corsivo_focal_point_autosave_failed',
			__( 'Il focal point non è stato salvato. Riprova prima di chiudere l’editor.', 'corsivo-focal-point' ),
			array( 'status' => 500 )
...
function corsivo_focal_point_finalize_revision_restore( $post_id ) {
	...
	if ( $snapshot['preserve'] ) {
		foreach ( corsivo_focal_point_position_meta_keys() as $meta_key ) {
			if ( array_key_exists( $meta_key, $snapshot['values'] ) ) {
				corsivo_focal_point_write_single_meta( $post_id, $meta_key, $snapshot['values'][ $meta_key ] );
			} else {
		...
		if ( $revision_state['has_position'] || $revision_state['has_attachment_link'] ) {
			corsivo_focal_point_update_position(
				$post_id,
				$revision_state['x'],
				$revision_state['y'],
